...
namespace OCC
{
    public partial class Addresses : UserControl
    {
        ...
        public Addresses(string user)
        {
            InitializeComponent();
            Addresses.username = user;
        }
        ...
        {
            textBox1.DropDownHeight = 100;
            textBox1.DroppedDown = true;
            var connectionString = DbConnect.conString;
            using (var connection = new MySqlConnection(connectionString))
            {
                connection.Open();
                var query = "SELECT NameOfArea FROM Areas WHERE NameOfArea LIKE '%" + textBox1.Text + "%' ORDER BY NameOfArea";
                using (var command = new MySqlCommand(query, connection))
                {
                    using (var reader = command.ExecuteReader())
                    {
